...
def extract_items(text):
    ...
    for line in lines:
        ...
        if any(keyword in line.lower()
               for keyword in ["subtotal", "total", "items"]):
            logger.debug(
                f"Skipping line containing subtotal/total/items: {line}")
            continue
...
def suggest_alternatives(items):
    ...
    for item in items:
        ...
        category = next((cat for cat in alternative_products.keys()
                         if cat.lower() in original_name.lower()), "Default")
        ...
        if category == "Default":
            alternative_price = round(
                original_price * random.uniform(0.8, 0.95), 2)
        else:
